...
from bizsim.engine import TickEngine, AgentProtocol, PendingQuery
...
class MockAgent:
    ...
    def emit_query(self, query_template: str, params: dict[str, Any], context: dict[str, Any]) -> QueryRequest:
        query_id = str(uuid4())
        req = QueryRequest(
            query_id=query_id,
...
def test_engine_ch3_query_pipeline():
    ...
    def mock_query_handler(req: QueryRequest) -> QueryResult:
        return QueryResult(req.query_id, req.agent_id, req.query_template, req.tick_issued, 2, {"data": "ok"})
...
def test_determinism():
    def run_sim(seed):
        ...
        for i in range(3):
            ...
            a1.actions_to_emit = [ActionEvent(uuid4(), "act", 1, "t1", engine.current_tick + 1, messages=[msg])]
            engine.step()
...
def test_dual_mode_reads_orchestration():
    ...
    def mock_query_handler(req):
        return QueryResult(req.query_id, req.agent_id, req.query_template, req.tick_issued, 2, {"mode": 2})
