...
| **Community / Social** | Not an agent — a simulation engine subsystem | A simulation engine subsystem that runs as a batch computation step in the tick loop (P3 step 4). Activated when consumers emit `SharePurchase` messages. Modifies other consumers' interest profiles and trend multipliers. |
...
**Consumer Order Status Lifecycle**:
requested → accepted → delivered
            → cancel_requested → cancelled (V2: seller confirms)
...
2. **Tick N+1**: Receive `QueryResult` items in inbox. Update local sales cache. Construct LLM context (see §9 for prompt template).
...
### 3.6 Community Subsystem (Non-Agent)

The Community Subsystem is a batch computation that runs once per tick as step 4 of the simulation tick loop (see vision.md P3). It is NOT an agent — it has no inbox, emits no Ch.2 messages, and does not participate in the agent scheduling system. It reads from a dedicated activation queue and directly mutates consumer agent state.
...
class CommunitySubsystem:
    ...
    def run_propagation(self, tick: int, consumers: dict[int, ConsumerAgent]) -> list[ActionEvent]:
        """Called by tick loop step 4. Returns Ch.1 events for edge weight updates."""
...
Note: The community subsystem (§3.6) is NOT scheduled via this mechanism. It runs as tick loop step 4, after all agent actions complete, and before events are emitted (step 6). See vision.md P3 for the full tick loop.
...
- Messages from the same tick are processed in a deterministic, reproducible order
- An agent always has fresh data context before processing action-triggering messages

### 4.5 Multi-Tick Pipeline Correlation
...
Note: `SharePurchase` is not a Ch.2 message. It is delivered via direct function call to the Community Subsystem (see §3.6).
...
| **Pipeline stall** | Agent waiting for QueryResult that never arrives | Tick-based TTL on pending operations (QUERY_TTL = 10 ticks, see §4.5) | Log warning, cancel pending operation after TTL ticks |

Note: Inbox processing follows a deterministic ordering protocol (see §4.4) to ensure reproducible behavior even under varying message delivery patterns.
...
| Valid supplier | Supplier must be mapped to the SKU in `sku_supplier_mapping` table (see §2) |
...
| **Order cancellation negotiation** | `Consumer.cancel_order()` | Cancel always succeeds if order not yet shipped; seller auto-accepts | Cancellation negotiation: seller may counter-offer, partial refund, restocking fee. Consumer decides whether to accept counter-offer. |
| **Community edge weight dynamics** | `CommunitySubsystem.run_propagation()` | Edges strengthen by fixed delta (+0.01) on successful propagation | Dynamic edge weight evolution: edges decay from disuse, strengthen from repeated successful influence, can be severed by negative experiences (bad reviews shared). |
